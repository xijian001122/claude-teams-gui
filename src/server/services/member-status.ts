...
export class MemberStatusService {
  ...
  private stateMap: Map<string, Map<string, MemberState>> = new Map();
   ...
   * Set member state
   */
  private setMemberState(teamName: string, memberName: string, status: MemberStatus, timestamp: number): void {
    if (!this.stateMap.has(teamName)) {
      this.stateMap.set(teamName, new Map());
    }
    const teamMap = this.stateMap.get(teamName)!;
  ...
  getMemberStatuses(teamName: string): MemberStatusInfo[] {
    ...
    const now = Date.now();
    const statuses: MemberStatusInfo[] = [];
   ...
   */
  private computeStatus(state: MemberState, now: number): MemberStatus {
   ...
   */
  private updateComputedStatus(teamName: string, memberName: string): void {
    const teamMap = this.stateMap.get(teamName);
    if (!teamMap) return;
